...
def validation(X,Y, TestSize = 0.3, epo = 10,prevalence = 0.5):
    ...
    #Build DNN
    var1 = tf.keras.layers.Input(shape=(4,))
    x1 = tf.keras.layers.Dense(250,activation = 'relu')(var1)
    x1 = tf.keras.layers.Dense(125,activation = 'relu')(x1)
    
    #second input model
    var2 = tf.keras.layers.Input(shape=(4,))
    x2 = tf.keras.layers.Dense(250,activation = 'relu')(var2)
    x2 = tf.keras.layers.Dense(125,activation = 'relu')(x2)
    
    #third input model
    var3 = tf.keras.layers.Input(shape=(10,))
    x3 = tf.keras.layers.Dense(250,activation = 'relu')(var3)
    x3 = tf.keras.layers.Dense(125,activation = 'relu')(x3)
    
    #merge the three inputs
    merge = tf.keras.layers.concatenate([x1, x2, x3])
    hidden = tf.keras.layers.Dense(15, activation='relu')(merge)
    output = tf.keras.layers.Dense(1, activation='sigmoid')(hidden)
    
    NewModelWOPain = tf.keras.models.Model(inputs=[var1,var2,var3], outputs=output)
    
    NewModelWOPain.compile(optimizer='sgd',
                    loss=tf.keras.losses.BinaryCrossentropy(),
                    metrics=['accuracy'])
    
    NewModelWOPain.fit([X_train1,X_train2,X_train3], Y_train, batch_size = 64,
                       epochs=epo,verbose=0)
    
    _, acc_bench = NewModelWOPain.evaluate([X_test1,X_test2,X_test3], Y_test,verbose=0)
    
    #Threshold = 0.5
    predict1prob_bench = NewModelWOPain.predict([X_test1,X_test2,X_test3])
    ...
    for i in range(5):
        ...
        X_train1_Balance = X_train_Balance[:,:4]
        X_train2_Balance = X_train_Balance[:,4:8]
        X_train3_Balance = X_train_Balance[:,8:18]
        ...
        NewModelWOPain.fit([X_train1_Balance,X_train2_Balance,X_train3_Balance], Y_train_Balance, 
                           batch_size = 64, epochs=epo,verbose=0)
        
        pred_balance = NewModelWOPain.predict([X_test1,X_test2,X_test3])
...
pickle.dump(ModelBasedResults,open("DNN_2Layer_250_v3.p","wb"))
