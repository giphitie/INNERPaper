...
def validation(X,Y, TestSize = 0.3, epo = 10,prevalence = 0.5):
    ...
    Z = tf.keras.layers.Input(shape = (17,),dtype = tf.float32)
    ...
    Benchmodel.fit([X_trainX,X_trainZ], Y_train,batch_size =64,epochs=epo,
                      verbose=0)
    ...
    for i in range(5):
        ...
        Benchmodel.compile(optimizer='sgd',
                      loss=tf.keras.losses.BinaryCrossentropy(),
                      metrics=['accuracy'])
        Benchmodel.fit([X_trainX_Balance,X_trainZ_Balance], Y_train_Balance,
                    batch_size =64,epochs=epo,verbose=0)
        
        pred_balance = Benchmodel.predict([X_testX,X_testZ])
...
pickle.dump(ModelBasedResults,open("LogReg_v3.p","wb"))
