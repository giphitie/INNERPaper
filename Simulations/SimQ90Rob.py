...
def validation(X,Y, TestSize = 0.2, epo = 10):
    ...
    AlphaZ = tf.keras.layers.Dense(400,activation = 'relu',name = 'AlphaZ_1')(WOPain)
    AlphaZ = tf.keras.layers.Dropout(rate = 0.25,name = 'AlphaZ_Dropout_1')(AlphaZ)
    AlphaZ = tf.keras.layers.Dense(400,activation = 'relu',name = 'AlphaZ_2')(AlphaZ)
    AlphaZ = tf.keras.layers.Dropout(rate = 0.25,name = 'AlphaZ_Dropout_2')(AlphaZ)
    AlphaZ = tf.keras.layers.Dense(1, activation = 'linear',name = 'AlphaZ_3')(AlphaZ)
    ...
    BetaZ = tf.keras.layers.Dense(400,activation = 'relu',name = 'BetaZ_1')(WOPain)
    BetaZ = tf.keras.layers.Dropout(rate = 0.25,name = 'BetaZ_Dropout_1')(BetaZ)
    BetaZ = tf.keras.layers.Dense(400,activation = 'relu',name = 'BetaZ_2')(BetaZ)
    ...
    LRmodel = tf.keras.models.Model(inputs=[Pain,WOPain,OnesA,OnesB], outputs=y_pred)
    LRmodel.compile(optimizer='rmsprop',
                  loss='categorical_crossentropy',
                  metrics=['accuracy'])
    LRmodel.fit([X_trainX,X_trainZ,X_trainOnesA,X_trainOnesB], Y_train_cat,epochs=epo,
                batch_size = 64,verbose=0)
    
    _, acc_lr = LRmodel.evaluate([X_testX,X_testZ,X_testOnesA,X_testOnesB], Y_test_cat,verbose=0)
    pre_lr = LRmodel.predict([X_testX,X_testZ,X_testOnesA,X_testOnesB])
    ...
    X = tf.keras.layers.Dense(400,activation = 'relu')(Input)
...
Y = np.genfromtxt("SimData_Y_q90_rob1.csv",delimiter=",",skip_header=1)
X = np.genfromtxt("SimData_X_q90_rob1.csv",delimiter=",",skip_header=1)
...
for i in range(0,Times):
    ...
    if (i+1)%100 == 0:
        ...
        pickle.dump(SimQ90Results,open(name1+str(i+1)+name3,"wb"))
